-- Signers may adjust their fields only when the sender allows it for the envelope
-- (off by default), and never "Date signed", which fills itself in. Envelopes and
-- templates that already exist keep allowing it, as they did when they were sent.
alter table public.envelopes add column allow_signer_adjustments boolean not null default true;
alter table public.envelopes alter column allow_signer_adjustments set default false;
alter table public.templates add column allow_signer_adjustments boolean not null default true;
alter table public.templates alter column allow_signer_adjustments set default false;
comment on column public.envelopes.allow_signer_adjustments is
  'Whether signers may nudge or resize their own fields (within limits) while signing.';

grant insert (allow_signer_adjustments), update (allow_signer_adjustments) on public.envelopes to authenticated;
grant insert (allow_signer_adjustments) on public.templates to authenticated;

-- Drafts save the setting (the eight-argument version is dropped once the app calling it is replaced)
...
revoke all on function public.save_envelope_draft(uuid, text, text, text, integer, integer, boolean, jsonb, jsonb) from public, anon;
grant execute on function public.save_envelope_draft(uuid, text, text, text, integer, integer, boolean, jsonb, jsonb) to authenticated;

-- The signing page needs the setting; the database enforces it
